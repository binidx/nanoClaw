...
describe('run-migrations', () => {
  ...
  function executeMigrations(
    ...
    try {
      const stdout = execFileSync(nodeBin, ['--import', tsxImportPath, scriptPath, from, to, newCoreDir], {
        cwd: tmpDir,
        encoding: 'utf-8',
        stdio: 'pipe',
        timeout: 30_000,
  ...
  it('outputs empty results when no migrations directory exists', () => {
    const { stdout, exitCode } = executeMigrations('1.0.0', '2.0.0');
...
`,
    ...
    const { stdout } = executeMigrations('1.0.0', '1.1.0');
    const result = JSON.parse(stdout);
...
`,
    ...
    const { stdout } = executeMigrations('1.0.0', '2.0.0');
    const result = JSON.parse(stdout);
