...
export function AppsPageV2({ apiBase, isAdmin }: AppsPageV2Props) {
  ...
  const [activeTab, setActiveTab] = useNavigatedTab<AppsV2Tab>('apps', VALID_TABS, 'my-apps');
  ...
  const tabs: Array<{ id: AppsV2Tab; label: string; count: string }> = [
    {
      ...
      count: t('auto.14e79b5c', { count: mcp.myServers.length + skills.mySkills.length }),
    },
    {
      id: 'store',
  ...
  const handleLibraryInstall = async (itemId: string, itemType: 'mcp' | 'skill') => {
  ...
  return (
    <div className="page-view apps-v2-page">
      ...
      <div className="page-body">
        ...
        {(() => {
          const errors = [mcp.error, skills.error, library.error].filter(Boolean);
