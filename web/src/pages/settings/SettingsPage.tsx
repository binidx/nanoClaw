  ...
  return (
    ...
    >
      <div className="page-header">
        <h2>{s.pageTitle}</h2>
        {!s.hideSettingsTabs && (
          <div className="settings-tabs" role="tablist" aria-label={t('settings.page.navigation')}>
            {(
              [
                ['providers', t('settings.page.AI_模型')],
                ['channels', t('settings.page.channels')],
                ['prompt', t('settings.page.prompt')],
                ['web-search', t('settings.page.webSearch')],
                ['general', t('settings.page.general')],
                ['knowledge', t('settings.page.知识库')],
                ['subagent', t('settings.page.子代理')],
                ['security', t('settings.page.security')],
                ['diagnostics', t('settings.page.diagnostics')],
                ['browser', t('settings.page.browser')],
                ['live2d', t('settings.page.live2d')],
                ['ssh-keys', t('settings.page.sshKeys')],
                ['trash', t('settings.page.回收站')],
                ['audit-log', t('settings.page.审计日志')],
              ] as [SettingsTab, string][]
            )
              .filter(([key]) => !s.visibleTabs || s.visibleTabs.includes(key))
              .map(([key, label]) => (
                <button
                  key={key}
                  type="button"
                  role="tab"
                  aria-selected={s.settingsTab === key}
                  className={`settings-tab ${s.settingsTab === key ? 'active' : ''}`}
                  onClick={() => s.setSettingsTab(key)}
                >
                  {label}
                </button>
              ))}
        ...
        {s.settingsTab === 'general' && (
          <Suspense fallback={tabFallback}>
            <SettingsGeneralTab
              ...
              defaultDirectoryTemplateCount={s.defaultDirectoryTemplateCount}
              ...
              renderBasicConfigField={s.renderBasicConfigField}
              DEFAULT_ACCESS_POLICY_OPTIONS={s.DEFAULT_ACCESS_POLICY_OPTIONS}
            />
          </Suspense>
        )}
        ...
        {s.settingsTab === 'security' && (
          <Suspense fallback={tabFallback}>
            <SettingsSecurityTab
              ...
              setSenderTrustOverridesError={s.setSenderTrustOverridesError}
              ...
              bashApprovalAllowlistMessage={s.bashApprovalAllowlistMessage}
              setBashApprovalAllowlistMessage={s.setBashApprovalAllowlistMessage}
        ...
        {s.settingsTab === 'diagnostics' && (
          <Suspense fallback={tabFallback}>
            <SettingsDiagnosticsTab
              ...
              refreshWorkspaceCleanupSummary={s.refreshWorkspaceCleanupSummary}
