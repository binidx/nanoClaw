...
import { IconChevronDown } from '../AppIcons';
import type {
...
function formatProfileBranchSummary(
  ...
  if (!profile.targetBranches.length) {
    return i18n.t('profile.activeBranches', { days: activeBranchWindowDays, ns: 'repoReview' });
  }
...
export function RepoReviewProfileSection({
  ...
  const branchSelectOptions = filteredProfileBranches
    ...
    .map((branch) => ({
      value: branch.name,
      label: branch.defaultBranch ? `${branch.name}${t('profile.baseline')}` : branch.name,
    }));
  ...
  return (
    <div className="repo-review-card repo-review-fold">
      ...
      >
        ...
        <span className={`repo-review-fold-icon ${sectionOpen ? 'open' : ''}`}>
          <IconChevronDown />
        </span>
      </button>
      <div className="repo-review-fold-card-actions">
        <button
          ...
          onClick={() => onOpenEditor(!profileDraft.id)}
          disabled={!selectedRepositoryId}
        >
          {profileDraft.id ? t('profile.edit') : t('profile.create')}
        </button>
      </div>
      {sectionOpen || editorOpen ? (
        <>
          {editorOpen ? (
            <button
              type="button"
              className="modal-overlay repo-review-editor-overlay"
              aria-label={t('profile.closeEditor')}
              onClick={onCloseEditor}
            />
          ) : null}
          <div
            className={`repo-review-fold-body ${editorOpen ? 'repo-review-editor-modal' : ''}`}
          >
            {editorOpen ? (
              <div className="repo-review-editor-modal-header">
                <div className="repo-review-fold-copy">
                  <h4>{profileDraft.id ? t('profile.editTitle') : t('profile.createTitle')}</h4>
                  <div className="settings-hint">
                    {t('profile.editorHint')}
                  </div>
                </div>
                <button
                  type="button"
            ...
            {!selectedRepositoryId ? (
              ...
              <>
                ...
                <div className="repo-review-form-grid">
                  ...
                  <div className="form-group repo-review-textarea">
                    ...
                    <div className="repo-review-branch-box">
                      <div className="settings-hint">
                        {t('profile.targetBranchesHint', { days: activeBranchWindowDays })}
                      </div>
                      <div className="repo-review-inline-actions">
                        ...
                        >
                          {loadingRemoteBranches ? t('profile.refreshing') : t('profile.refreshBranches')}
                        </button>
                      </div>
                      <div className="settings-hint">
                        {availableProfileBranches.length > 0
                          ? t('profile.branchesFound', { count: availableProfileBranches.length })
                      ...
                      {profileDraft.targetBranches.length > 0 ? (
                        <div className="settings-hint">
                          {t('profile.selectedBranches', { count: profileDraft.targetBranches.length })}
                        </div>
                      ) : (
                        <div className="settings-hint">
                ...
                <div className="form-group">
                  ...
                  />
                  <div className="settings-hint">
                    {profileDraft.reviewOutputMode === 'message' && t('profile.outputMessageHint')}
                    {profileDraft.reviewOutputMode === 'share_link' && t('profile.outputShareLinkHint')}
                  </div>
                </div>
