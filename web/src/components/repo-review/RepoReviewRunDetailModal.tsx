      ...
      >
        ...
        <div className="repo-review-run-detail-body">
          ...
          <div className="repo-review-run-badges">
            ...
            >
              {t('runDetail.notification')} {formatDeliveryStatusLabel(chatDeliveryStatus)}
            </span>
            <span
              className={`repo-review-source-pill tone-${getDeliveryTone(
            ...
            >
              {t('runDetail.platform')} {formatDeliveryStatusLabel(platformDeliveryStatus)}
            </span>
            {run.manualDecision ? (
              <span className="repo-review-source-pill tone-warning">
                {t('runDetail.manual')} {run.manualDecision === 'pass' ? t('runDetail.pass') : t('runDetail.fail')}
              </span>
            ) : null}
          </div>
          <div className="status-detail-grid repo-review-run-detail-grid">
            ...
            <div className="status-detail-item">
              <span className="status-detail-label">{t('runDetail.stageSource')}</span>
              <strong className="status-detail-value">
                {formatRunStageLabel(run.stage)} · {formatRunSourceLabel(run.source)}
              </strong>
            </div>
            <div className="status-detail-item">
              <span className="status-detail-label">{t('runDetail.branchRef')}</span>
              <strong className="status-detail-value">
                {run.branch || '-'}
                {run.ref ? ` · ${run.ref}` : ''}
              </strong>
            ...
            <div className="status-detail-item">
              <span className="status-detail-label">{t('runDetail.baselineSource')}</span>
              <strong className="status-detail-value">
            ...
            <div className="status-detail-item">
              <span className="status-detail-label">{t('runDetail.commitRange')}</span>
              <strong className="status-detail-value">
                {formatShortSha(run.baseSha)} {'->'} {formatShortSha(run.headSha)}
              </strong>
            </div>
            <div className="status-detail-item">
              <span className="status-detail-label">{t('runDetail.duration')}</span>
              <strong className="status-detail-value">
                {formatDurationMs(getRunDurationMs(run))}
              </strong>
            </div>
            <div className="status-detail-item">
              <span className="status-detail-label">{t('runDetail.manualDecision')}</span>
              <strong className="status-detail-value">
                {run.manualDecision
            ...
            <div className="status-detail-item">
              <span className="status-detail-label">{t('runDetail.chatDelivery')}</span>
              <strong className="status-detail-value">
                {formatDeliveryStatusLabel(chatDeliveryStatus)}
                {run.deliveryRetryCount ? t('runDetail.deliveryRetry', { count: run.deliveryRetryCount }) : ''}
              </strong>
            </div>
            <div className="status-detail-item">
              <span className="status-detail-label">{t('runDetail.platformWrite')}</span>
              <strong className="status-detail-value">
                {formatDeliveryStatusLabel(platformDeliveryStatus)}
                {run.platformCommentUrl ? t('runDetail.commentLinkGenerated') : ''}
              </strong>
            </div>
            <div className="status-detail-item">
              <span className="status-detail-label">{t('runDetail.createdAt')}</span>
              <strong className="status-detail-value">
                {new Date(run.createdAt).toLocaleString()}
              </strong>
            </div>
            <div className="status-detail-item">
              <span className="status-detail-label">{t('runDetail.completedAt')}</span>
              <strong className="status-detail-value">
                {run.completedAt
            ...
            {run.idempotencyKey ? (
              <div className="status-detail-item">
                <span className="status-detail-label">{t('runDetail.idempotencyKey')}</span>
                <strong className="status-detail-value">{run.idempotencyKey}</strong>
              </div>
            ) : null}
            {run.platformCommentUrl ? (
              <div className="status-detail-item">
                <span className="status-detail-label">{t('runDetail.platformComment')}</span>
                <strong className="status-detail-value">
                  <a href={run.platformCommentUrl} target="_blank" rel="noreferrer">
          ...
          <div className="repo-review-run-detail-stack">
            {run.summary ? (
              <RepoReviewDetailCard title={t('runDetail.runSummary')} summary={run.summary}>
            ...
            {progressEntries.length > 0 ? (
              <RepoReviewDetailCard
                title={t('runDetail.aiAnalysis', { count: progressEntries.length })}
                summary={
                  run.status === 'running'
                ...
                <div className="repo-review-run-findings">
                  {run.findings.map((finding, index) => (
                    <div key={`${run.id}-finding-${index}`}>
                      [{finding.severity}] {finding.file ? `${finding.file}: ` : ''}
                      {finding.title}
            ...
            {run.commitReviews.length > 0 ? (
              <RepoReviewDetailCard
                title={t('runDetail.commitReviews', { count: run.commitReviews.length })}
              ...
              <RepoReviewDetailCard
                title={t('runDetail.commitList', { count: run.commitDetails.length })}
            ...
            {run.changedFiles.length > 0 ? (
              <RepoReviewDetailCard
                title={t('runDetail.changedFiles', { count: run.changedFiles.length })}
            ...
            {run.suggestions.length > 0 ? (
              <RepoReviewDetailCard
                title={t('runDetail.suggestions', { count: run.suggestions.length })}
