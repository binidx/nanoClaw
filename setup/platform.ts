...
export type ServiceManager = 'launchd' | 'systemd' | 'windows-task' | 'none';
...
export function isRoot(): boolean {
  if (isWin) return isWindowsAdmin();
  return process.getuid?.() === 0;
}

/**
 * Check if the current Windows process has administrator privileges.
 * Runs `net session` which only succeeds under an elevated prompt.
 */
...
export function getServiceManager(): ServiceManager {
  ...
  if (platform === 'windows') return 'windows-task';
  if (platform === 'linux') {
...
/**
 * The null device path for the current OS.
 * Used to shadow files in agent path mappings.
 */
export const DEV_NULL = isWin ? 'NUL' : '/dev/null';
