...
export async function run(_args: string[]): Promise<void> {
  ...
  } else if (mgr === 'windows-task') {
    // Windows: check Task Scheduler + PID file
    try {
      const output = execSync('schtasks /Query /TN "NanoClaw" /FO LIST', {
        encoding: 'utf-8',
        shell: 'cmd.exe',
      });
      ...
      // Task not found; fall through to PID file check
    }
    // Also verify via PID file (task may have started the process)
