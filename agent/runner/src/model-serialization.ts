...
function maybeLogSerialization(
  ...
  console.error(
    `[model-serialization] surface=${surface} format=${format} ` +
      `jsonTokens=${stats.jsonTokens} toonTokens=${stats.toonTokens ?? 'n/a'} ` +
      `outputTokens=${stats.outputTokens} savingsRatio=${stats.savingsRatio.toFixed(3)}`,
  );
}
