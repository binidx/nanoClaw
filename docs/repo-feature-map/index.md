...
| “Repo Review 超预算/分支同步/digest” | `docs/RepoReview.md`, `src/repo-review/repo-review-run-executor.ts`, `src/repo-review/repo-review-digest-service.ts`, `src/repo-review/repo-review-sync-service.ts`, `web/src/components/repo-review/*`；如果问题是“列表里为什么没看到 tool-call / turn 流”，同时查 `src/repo-review/repo-review-read-service.ts` 和 detail 接口 | 不要把 CodeMap 和 Repo Review 混在一起改。 |
