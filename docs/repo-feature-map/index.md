...
- 代码源：`remote_worktree`，`source_branch=main`，`source_head_sha=a9edaaebf39962cae0a8eb5fea4a449b270fd377`。
...
| 助手、Provider、模型与运行配置 | `src/routes/assistant-routes.ts`, `src/routes/capability-routes.ts`, `src/routes/available-provider-routes.ts`, `src/routes/user-provider-routes.ts`, `src/routes/runtime-customization-routes.ts` | `src/assistant/assistant-config.ts`, `src/assistant/assistant-runtime.ts`, `src/capabilities/capability-catalog.ts`, `src/provider/provider-api.ts`, `src/provider/provider-registry.ts`, `src/provider/provider-adapters.ts`, `src/provider/provider-http-config.ts`, `src/runtime/runtime-customization.ts`, `src/agent/agent-runner-spawn.ts`, `src/db/assistants.ts`, `src/db/config.ts`, `src/db/schema-*.ts` | `web/src/pages/AssistantsPage.tsx`, `web/src/pages/settings/SettingsProvidersTab.tsx`, `web/src/pages/settings/useSettingsPageModel.tsx` | `src/capability-routes.test.ts`, `src/provider/provider-api.test.ts`, `src/provider/provider-adapters.test.ts`, `src/runtime/runtime-customization.test.ts`, `src/db.test.ts` |
...
| 项目索引图谱 | `src/routes/repository-routes.ts`, `src/routes/assistant-routes.ts`, `src/project-graph/project-graph-service.ts`, `src/db/project-graph.ts`, `src/assistant/assistant-runtime.ts` | `src/db/repositories.ts` 的 `repo_features(feature_type='project_graph')`, `src/db/schema-*.ts` 的 `project_graph_*` 表, `src/db/code-index-db.ts` | `web/src/components/repository/ProjectGraphPanel.tsx`, `web/src/components/RepoReviewSettingsPanel.tsx`, `web/src/pages/RepositoryPage.tsx`, `web/src/pages/AssistantsPage.tsx` | `src/project-graph/project-graph-service.test.ts`, `src/assistant/assistant-runtime.test.ts`, `src/assistant-routes.test.ts`；当前 MVP 生成项目画像、服务/配置关系、服务依赖候选、库表候选和文档草稿，并把项目图谱作为 Assistant 仓库绑定的通用上下文、Skill/MCP 资源推荐与可同步组合资源接入 |
...
| `src/routes/capability-routes.ts` | 通用能力目录，输出 Assistant、仓库、项目图谱、Skills、MCP、Workflow 节点与关系 | `src/capabilities/capability-catalog.ts`, `src/db/assistants.ts`, `src/db/workflows.ts`, `src/repo-review/repository-service.ts` |
