...
- 代码源：`remote_worktree`，`source_branch=main`，`source_head_sha=da416dcbb9e43d72a8fa703af18d9d538bbe1d93`。
...
| 项目索引图谱 | `src/routes/repository-routes.ts`, `src/project-graph/project-graph-service.ts`, `src/db/project-graph.ts`, `src/assistant/assistant-runtime.ts` | `src/db/repositories.ts` 的 `repo_features(feature_type='project_graph')`, `src/db/schema-*.ts` 的 `project_graph_*` 表, `src/db/code-index-db.ts` | `web/src/components/repository/ProjectGraphPanel.tsx`, `web/src/components/RepoReviewSettingsPanel.tsx`, `web/src/pages/RepositoryPage.tsx`, `web/src/pages/AssistantsPage.tsx` | `src/project-graph/project-graph-service.test.ts`, `src/assistant/assistant-runtime.test.ts`；当前 MVP 生成项目画像、服务/配置关系、服务依赖候选、库表候选和文档草稿，并把项目图谱作为 Assistant 仓库绑定的通用上下文与 Skill/MCP 资源提示接入 |
