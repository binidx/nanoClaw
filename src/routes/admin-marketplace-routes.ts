...
export function registerAdminMarketplaceRoutes(
  ...
  const adminGuard = opts.requirePermission('admin.settings.write', 'marketplace.manage_sources');

  app.get('/api/admin/marketplace-sources', adminGuard, async (_req: Request, res: Response) => {
    try {
  ...
  app.get('/api/admin/marketplace-sources/:id', adminGuard, async (req: Request, res: Response) => {
    try {
  ...
  app.post('/api/admin/marketplace-sources', adminGuard, async (req: Request, res: Response) => {
    try {
      const userId = getCurrentUserId();
      const { name, source, enabled, description, iconUrl, sortOrder } = req.body;
      if (!name || !source) {
  ...
  app.put('/api/admin/marketplace-sources/:id', adminGuard, async (req: Request, res: Response) => {
    try {
      const result = await updateMarketplaceSource(paramStr(req.params.id), req.body);
      if (!result) {
        res.status(404).json({ error: 'Marketplace source not found' });
        return;
      }
  ...
  app.delete('/api/admin/marketplace-sources/:id', adminGuard, async (req: Request, res: Response) => {
    try {
