...
} from '../code-intelligence/code-map-builder.js';
import {
  listCandidateFiles,
...
import {
  renderCodeMapText,
  renderCodeMapSummary,
} from '../code-intelligence/code-map-render.js';
import {
...
const repoDescriptionInFlight = new Map<
  string,
  Promise<{ description: RepoDescription; noAi?: boolean }>
>();
...
export function registerCodeMapRoutes(
  ...
  app.get('/api/code-map/repositories', guard, async (req, res) => {
    try {
      ...
      for (const r of repos) {
        if (!(await canAccessRepositoryResource(getTenantUserId(req), r.id)))
          continue;
  ...
  app.get('/api/code-map/:repositoryId/text', guard, async (req, res) => {
    try {
      ...
      if (!snapshot) {
        res
          .status(404)
          .json({ error: t('errors.auto_c0c012', {}, req.locale) });
        return;
      }
  ...
  app.post(
    '/api/code-map/:repositoryId/rebuild',
    manageGuard,
    async (req, res) => {
      try {
        ...
        if (!rootDir) {
          res
            .status(400)
            .json({
              error: t('errors.codemap.noWorkDir', { branch }, req.locale),
            });
          return;
        }
        ...
        try {
          ...
          if (!force) {
            ...
            if (existing) {
              ...
              const currentHash = computeCodeMapManifestHash(
                rootDir,
                manifestEntries,
              );
          ...
          const snapshot = await buildCodeMapAsync(
            rootDir,
            repositoryId,
            branch,
          );
  ...
  app.post(
    '/api/code-map/:repositoryId/ai-summary',
    guard,
    async (req, res) => {
      try {
        const repositoryId = decodeURIComponent(
        ...
        if (!filePath && !dirPath) {
          res
            .status(400)
            .json({ error: t('errors.auto_3032c9', {}, req.locale) });
          return;
        }
        ...
        while (aiSummaryCache.size > 200) {
          ...
          if (k !== undefined) aiSummaryCache.delete(k);
          else break;
        }
        ...
        if (isNoProvider) {
          res
            .status(501)
            .json({ error: t('errors.auto_9d995b', {}, req.locale) });
          return;
        }
        codeMapLog.error({ err }, 'code-map: AI summary error');
        res
          .status(500)
          .json({ error: t('errors.auto_741cff', {}, req.locale) });
      }
    },
  );
  ...
  function truncSig(sig: string, max: number): string {
    ...
    if (parenIdx > 0 && parenIdx < max) {
      ...
      if (closeIdx > 0 && closeIdx <= max + 20)
        return sig.slice(0, closeIdx + 1);
    }
  ...
  async function buildAnalysisContext(
    ...
    if (filePath) {
      ...
      const topSyms = [...file.symbols]
        .sort((a, b) => b.rank - a.rank)
        .slice(0, profile.maxSyms);
      const depEdges = snapshot.edges
        .filter((e) => e.fromFile === filePath)
        .slice(0, profile.maxDeps);
      const refEdges = snapshot.edges
        .filter((e) => e.toFile === filePath)
        .slice(0, profile.maxDeps);
      ...
      const lines: string[] = [
        ...
        ...topSyms.map(
          (s) =>
            `  - [${s.kind}] ${s.name} (${t('errors.codemap.line', { line: s.line }, locale)}) ${truncSig(s.signature, profile.sigMaxLen)}`,
        ),
      ];
      ...
      if (depEdges.length > 0) {
        ...
        for (const e of depEdges) {
          const depFile = snapshot.files.find(
            (f) => f.relativePath === e.toFile,
          );
          const symsStr =
            e.symbols.length > 0
              ? ` (${t('errors.codemap.referenced', {}, locale)}: ${e.symbols.join(', ')})`
              : '';
          if (depFile) {
            const depSyms = [...depFile.symbols]
              .sort((a, b) => b.rank - a.rank)
              .slice(0, profile.maxRelatedSyms);
            lines.push(`  ${e.toFile}${symsStr}`);
            lines.push(
              `    ${t('errors.codemap.keySymbols', {}, locale)}: ${depSyms.map((s) => `[${s.kind}] ${s.name} (${t('errors.codemap.line', { line: s.line }, locale)})`).join(', ')}`,
            );
          } else {
            lines.push(`  ${e.toFile}${symsStr}`);
          }
      ...
      if (refEdges.length > 0) {
        ...
        for (const e of refEdges) {
          const refFile = snapshot.files.find(
            (f) => f.relativePath === e.fromFile,
          );
          const symsStr =
            e.symbols.length > 0
              ? ` (${t('errors.codemap.referenced', {}, locale)}: ${e.symbols.join(', ')})`
              : '';
          if (refFile) {
            const refSyms = [...refFile.symbols]
              .sort((a, b) => b.rank - a.rank)
              .slice(0, profile.maxRelatedSyms);
            lines.push(`  ${e.fromFile}${symsStr}`);
            lines.push(
              `    ${t('errors.codemap.keySymbols', {}, locale)}: ${refSyms.map((s) => `[${s.kind}] ${s.name} (${t('errors.codemap.line', { line: s.line }, locale)})`).join(', ')}`,
            );
          } else {
            lines.push(`  ${e.fromFile}${symsStr}`);
          }
      ...
      if (depEdges.length + refEdges.length > 0) {
        ...
        for (const e of depEdges)
          lines.push(
            `  ${filePath} → ${e.toFile} (${e.symbols.join(', ') || t('errors.codemap.importedVia', {}, locale)})`,
          );
        for (const e of refEdges)
          lines.push(
            `  ${e.fromFile} → ${filePath} (${e.symbols.join(', ') || t('errors.codemap.importedVia', {}, locale)})`,
          );
      }
      ...
      if (codeIndexDetail?.file?.summary) {
        ...
        lines.push(
          `  ${truncateAnalysisText(codeIndexDetail.file.summary, 420)}`,
        );
      }
      ...
      if (topChunks.length > 0) {
        ...
        for (const chunk of topChunks) {
          ...
          lines.push(
            `    snippet: ${truncateAnalysisText(chunk.content, 240)}`,
          );
        }
      }
    ...
    if (dirPath) {
      const dirFiles = snapshot.files.filter((f) =>
        f.relativePath.startsWith(dirPath + '/'),
      );
      ...
      const topFiles = [...dirFiles]
        .sort((a, b) => b.rank - a.rank)
        .slice(0, 12);
      const internalEdges = snapshot.edges
        .filter(
          (e) =>
            e.fromFile.startsWith(dirPath + '/') &&
            e.toFile.startsWith(dirPath + '/'),
        )
        .slice(0, 20);
      const externalDeps = snapshot.edges
        .filter(
          (e) =>
            e.fromFile.startsWith(dirPath + '/') &&
            !e.toFile.startsWith(dirPath + '/'),
        ...
        .slice(0, 15);
      const externalRefs = snapshot.edges
        .filter(
          (e) =>
            !e.fromFile.startsWith(dirPath + '/') &&
            e.toFile.startsWith(dirPath + '/'),
      ...
      const codeIndexFilesByPath = new Map(
        (codeIndexSearch?.files || []).map((entry) => [
          entry.relativePath,
          entry,
        ]),
      );
      ...
      const lines: string[] = [
        t(
          'errors.codemap.targetDirectory',
          { dirPath, count: dirFiles.length },
          locale,
        ),
      ...
      if (internalEdges.length > 0) {
        ...
        for (const e of internalEdges)
          lines.push(
            `  ${e.fromFile} → ${e.toFile} (${e.symbols.join(', ') || t('errors.codemap.importedVia', {}, locale)})`,
          );
      }
      if (externalDeps.length > 0) {
        ...
        for (const e of externalDeps) {
          ...
          if (depFile) {
            const syms = [...depFile.symbols]
              .sort((a, b) => b.rank - a.rank)
              .slice(0, 3);
            lines.push(
              `  → ${e.toFile}: ${syms.map((s) => `[${s.kind}] ${s.name}`).join(', ')}`,
            );
          } else {
            lines.push(`  → ${e.toFile}`);
          }
  ...
  app.post(
    '/api/code-map/:repositoryId/ai-analysis',
    guard,
    async (req, res) => {
      let repositoryId = '';
      let branch = '';
      ...
      try {
        ...
        if (!forceRefresh) {
          ...
          if (memCached) {
            res.json({ analysis: memCached, cached: true });
            return;
          }

          const dbCached = await getCodeMapAiAnalysis(
            repositoryId,
            branch,
            targetPath,
        ...
        const activeKey = `analysis\0${repositoryId}\0${branch}\0${targetPath}\0${snapshot.manifestHash}`;
        if (aiAnalysisInFlight.has(activeKey)) {
          res.status(202).json({
            status: 'already_running',
            error: 'AI analysis is already running for this target.',
          });
          return;
        }
        ...
        aiAnalysisInFlight.add(activeKey);
        ...
        try {
          ...
          sendSSE('status', {
            message: t('errors.auto_4d2bb7', {}, req.locale),
          });
          ...
          while (aiAnalysisCache.size > 100) {
            ...
            if (k !== undefined) aiAnalysisCache.delete(k);
            else break;
          }
          ...
          saveCodeMapAiAnalysis({
            ...
            .then(() => pruneCodeMapAiAnalyses(repositoryId, 200))
            .catch((e) => {
              codeMapLog.error(
          ...
          const isNoProvider =
            ...
            err.message.includes('No default AI provider');
          sendSSE('error', {
            message: isNoProvider
              ? t('errors.auto_1c62f2', {}, req.locale)
              : t('errors.auto_ef3e0d', {}, req.locale),
          });
          if (!isNoProvider) {
            codeMapLog.error(
          ...
          aiAnalysisInFlight.delete(activeKey);
        }
        res.end();
      } catch (err) {
        if (!res.headersSent) {
          ...
          if (isNoProvider) {
            res
              .status(501)
              .json({ error: t('errors.auto_1c62f2', {}, req.locale) });
            return;
          }
          codeMapLog.error(
            { err, repositoryId, branch, targetPath, targetType },
          ...
          res
            .status(500)
            .json({ error: t('errors.auto_ef3e0d', {}, req.locale) });
        }
      }
    },
  );
  ...
  app.get(
    '/api/code-map/:repositoryId/file-content',
    guard,
    async (req, res) => {
      try {
        const repositoryId = decodeURIComponent(
        ...
        if (!filePath) {
          res
            .status(400)
            .json({ error: t('errors.auto_369996', {}, req.locale) });
          return;
        }
        ...
        if (!rootDir) {
          ...
          const recovered = await acquireWorktree({
            repositoryId,
            branch,
            cloneUrl,
            purpose: 'codemap',
        ...
        if (!rootDir) {
          res
            .status(404)
            .json({ error: t('errors.auto_b4ea78', {}, req.locale) });
          return;
        }
        ...
        if (!absPath) {
          res
            .status(400)
            .json({ error: t('errors.auto_640680', {}, req.locale) });
          return;
        }
        ...
        try {
          ...
          if (stat.size > 512 * 1024) {
            res
              .status(413)
              .json({ error: t('errors.auto_1c8479', {}, req.locale) });
            return;
          }
          ...
          res
            .status(404)
            .json({ error: t('live2d.fileNotFound', {}, req.locale) });
          return;
        }
  ...
  app.get(
    '/api/code-map/:repositoryId/repo-description',
    guard,
    async (req, res) => {
      try {
        const repositoryId = decodeURIComponent(
        ...
        if (memCached) {
          res.json({ description: memCached, cached: true });
          return;
        }
        ...
        res
          .status(500)
          .json({ error: t('errors.auto_c43130', {}, req.locale) });
      }
    },
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const streamResponse = !!req.body?.stream;
        ...
        if (!forceRefresh) {
          const inFlight = repoDescriptionInFlight.get(cacheKey);
          if (inFlight) {
        ...
        const buildFallbackDescription = (
          ...
          return {
            ...
            overview: '',
            techStack: rootDir ? extractTechStack(rootDir) : [],
            architecture: '',
        ...
        const saveDescription = (description: RepoDescription) => {
          ...
          while (repoDescriptionCache.size > 50) {
            ...
            if (k !== undefined) repoDescriptionCache.delete(k);
            else break;
          }
        ...
        const generateDescription = async (
          ...
          if (!rootDir) {
            return { description: buildFallbackDescription(null) };
          }
          ...
          try {
            if (onChunk) {
              const stream =
                await generateTextStreamWithDefaultProvider(prompt);
              for await (const chunk of stream) {
                aiText += chunk;
                onChunk(chunk);
              }
            ...
            if (isNoProvider) {
              return {
                description: buildFallbackDescription(rootDir),
                noAi: true,
              };
            }
            throw err;
          ...
          const description = assembleRepoDescription(
            snapshot,
            rootDir,
            aiResult,
          );
        ...
        if (streamResponse) {
          ...
          sendSSE('status', { message: '正在生成仓库概览...' });
          const generation = generateDescription((chunk) =>
            sendSSE('chunk', { text: chunk }),
          );
          repoDescriptionInFlight.set(cacheKey, generation);
          try {
            ...
            sendSSE('error', {
              message:
                err instanceof Error
                  ? err.message
                  : t('errors.auto_a197fe', {}, req.locale),
            });
          } finally {
            repoDescriptionInFlight.delete(cacheKey);
            res.end();
          }
          return;
        }

        const generation = generateDescription();
        repoDescriptionInFlight.set(cacheKey, generation);
        try {
          const result = await generation;
          res.json({
        ...
        res
          .status(500)
          .json({ error: t('errors.auto_a197fe', {}, req.locale) });
      }
    },
  );
  ...
  app.get('/api/code-map/:repositoryId/summary', guard, async (req, res) => {
    try {
      ...
      if (!snapshot) {
        res
          .status(404)
          .json({ error: t('errors.auto_23250d', {}, req.locale) });
        return;
      }
