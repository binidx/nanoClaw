 ...
 * This prevents direct-run agents from modifying security configuration.
 *
...
const logger = createModuleLogger('mount-security');
...
export function validateMount(
  ...
  const mountPath = mount.mountPath || path.basename(mount.hostPath);

  // Validate mount path (cheap check)
  if (!isValidMountPath(mountPath)) {
    return {
      allowed: false,
      reason: `Invalid mount path: "${mountPath}" - must be relative, non-empty, and not contain ".."`,
    };
  }
  ...
  for (const mount of mounts) {
    ...
    if (result.allowed) {
      validatedMounts.push({
        ...
        targetPath: `/workspace/extra/${result.resolvedMountPath}`,
      ...
      logger.debug(
        {
          group: groupName,
          hostPath: result.realHostPath,
          targetPath: result.resolvedMountPath,
          readonly: result.effectiveReadonly,
      ...
      logger.warn(
        {
          ...
          targetPath: mount.mountPath || path.basename(mount.hostPath),
          reason: result.reason,
        },
