...
describe('loadSenderAllowlist', () => {
  ...
  it('normalizes and saves sender trust config', () => {
    const saved = saveSenderAllowlist(
      {
        default: { allow: [' alice ', 'alice', 'bob'], mode: 'drop' },
        chats: {
          'web:test': {
            allow: [' sender_a ', 'sender_a', 'sender_b'],
            mode: 'trigger',
          },
        },
        logDenied: false,
    ...
    expect(saved).toEqual({
      ...
      chats: {
        'web:test': {
          allow: ['sender_a', 'sender_b'],
          mode: 'trigger',
        },
      },
      logDenied: false,
    });
    expect(loadSenderAllowlist(cfgPath('saved.json'))).toEqual(saved);
  });

  it('normalizes missing sender trust config to safe defaults', () => {
    expect(normalizeSenderAllowlistConfig(undefined)).toEqual({
      default: { allow: '*', mode: 'trigger' },
      chats: {},
      logDenied: true,
    });
  ...
  it('rejects invalid sender trust API payload shapes', () => {
    expect(() => assertSenderAllowlistConfigShape([])).toThrow(
      'sender trust config must be an object',
    );
    expect(() =>
      assertSenderAllowlistConfigShape({
        chats: {
    ...
    ).toThrow('sender trust chat override is invalid: group');
  });
